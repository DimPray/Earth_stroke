...
input_path = 'I:/nc_task/3_air-related/HiTIC_13-20_WGS84_monthly/'
output_path = 'I:/nc_task/3_air-related/HiTIC_13-20_WGS84_UTM_50N_yearly/'

names = ['ATin', 'ATout', 'DI', 'ET', 'HI', 'HMI', 'MDI', 'NET', 'SAT', 'sWBGT', 'WBT', 'WCT']

for year in range(2013, 2021):
    for name in names:
        rasters = [raster for raster in os.listdir(input_path) if raster.startswith(name) and raster.find(str(year))]
        print(rasters)

        for i in range(0, len(rasters)):
            # read
            dataset = gdal.Open(input_path + rasters[i])
        ...
        dst_ds = driver.Create(output_path + name + '_' + str(year) + '_wuhan_WGS84_UTM_50N_max.tif', im_width, im_height, im_bands, gdal.GDT_CFloat64)  # 创建文件
        dst_ds.SetProjection(projection)
        dst_ds.SetGeoTransform(geotrans)  # 设置几何信息
        dst_ds.GetRasterBand(1).WriteArray(array_max)  # 将数组写入

        driver = gdal.GetDriverByName('GTiff')  # 创建驱动
        dst_ds = driver.Create(output_path + name + '_' + str(year) + '_wuhan_WGS84_UTM_50N_mean.tif', im_width, im_height, im_bands, gdal.GDT_Float64)  # 创建文件
        dst_ds.SetProjection(projection)
        dst_ds.SetGeoTransform(geotrans)  # 设置几何信息
