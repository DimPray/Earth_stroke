...
input_path = 'I:/nc_task/4_biology-related/NDVI_13-20_WGS84_UTM_50N_16days/'
output_path = 'I:/nc_task/4_biology-related/NDVI_13-20_WGS84_UTM_50N_yearly/'
...
for year in years:
    ...
    dst_ds = driver.Create(output_path + 'NDVI_' + str(year) + '_wuhan_WGS84_UTM_50N_max.tif', im_width, im_height, im_bands, gdal.GDT_CFloat64)  # 创建文件
    dst_ds.SetProjection(projection)
    dst_ds.SetGeoTransform(geotrans)  # 设置几何信息
    dst_ds.GetRasterBand(1).WriteArray(array_max)  # 将数组写入

    driver = gdal.GetDriverByName('GTiff')  # 创建驱动
    dst_ds = driver.Create(output_path + 'NDVI_' + str(year) + '_wuhan_WGS84_UTM_50N_mean.tif', im_width, im_height, im_bands, gdal.GDT_CFloat64)  # 创建文件
    dst_ds.SetProjection(projection)
    dst_ds.SetGeoTransform(geotrans)  # 设置几何信息
