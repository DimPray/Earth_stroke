...
if leaf:
    ...
    def NC_to_tiffs(data, Output_folder):
        ...
        PM_arr = np.asarray(nc_data_obj.variables['PM2.5'])  # 这里根据需求输入想要转换的波段名称
        ...
        out_tif_name = Output_folder + '/' + 'PM25' + '_' + data.split('_')[5] + '_' + 'WGS84.tif'
    ...
    def main():
        Input_folder = 'I:/Hubei_database/0. Raw datasets/Pollutant/PM2.5/PM2.5_13-20/'
        Output_folder = 'I:/Hubei_database/0. Raw datasets/Pollutant/PM2.5/PM2.5_13-20_tif/'
        # 读取所有nc数据
...
if leaf:
    ...
    path_input = r"I:/Hubei_database/0. Raw datasets/Pollutant/PM2.5/PM2.5_13-20_tif/"
    path_output = r'I:/nc_task/3_air-related/PM25_13-20_WGS84_daily/'  # PM25_20130101_WGS84.tif

    for raster in os.listdir(path_input):
        # print(raster)
        input_raster = gdal.Open(path_input + '/' + raster)
        output_raster = path_output + '/' + 'PM25_' + raster.split('_')[1] + '_wuhan_WGS84.tif'
