...
if __name__ == "__main__":
    ...
    raster_path = 'I:/nc_task/3_air-related/HiTIC_13-20_WGS84_UTM_50N_yearly/mean_100m/'
    zones_path = 'I:/nc_task/shp/spatial_join_5km/'
    csv_path = 'I:/nc_task/3_air-related/HiTIC_13-20_WGS84_UTM_50N_yearly_zonal_csv/mean/'

    fn_zones = [shp for shp in os.listdir(zones_path) if shp.endswith('.shp')]
    names = ['ATin', 'ATout', 'DI', 'ET', 'HI', 'HMI', 'MDI', 'NET', 'SAT', 'sWBGT', 'WBT', 'WCT']
    for name in names:

        fn_raster = [raster for raster in os.listdir(raster_path) if raster.startswith(name) and raster.endswith('.tif')]

        for i in range(0, len(fn_raster)):
            fn_csv = csv_path + str(2013 + i) + '_zonal_statistics_5km_' + name + '_mean.csv'
